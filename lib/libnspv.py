...
class NspvRpcCalls:
    ...
    @staticmethod
    def get_response_arg(bytez, arg):  # expected arg = string(key)
        inp = json.loads(bytez.decode("utf-8"))
        outp = inp.get(arg)
        return outp
...
def def_credentials(chain):
    if chain == "KMD":
        ...
        print("Unknown chain: ", chain, "\nFall back to default chain: KMD")
        port = '7771'
    userpass = "userpass"
    url = ("http://127.0.0.1:" + port)  # TODO: enable coins file parse
    r = NspvRpcCalls(url, userpass)
    return r
